...
def RunCases():
  ...
  global N
  # N = 2 * random.randint(5, int(ORIGINAL_N / 2))
  N = ORIGINAL_N
  correct = 0
  for case_number in range(NUM_CASES):
    ...
    for move_number in range(K+1):
      ...
      if move[0] == "E":
        ...
        err = abs(estimate - correct_total_edges) / correct_total_edges
        if lo <= estimate and estimate <= hi:
          print(f"Case #{case_number}: Correct -- got {estimate}; exact answer is {correct_total_edges} (Error: {err:.2f})", file=sys.stderr)
          correct += 1
        else:
